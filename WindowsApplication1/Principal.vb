...
Public Class frm_Principal
    ...
    Private Sub btt_Eliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btt_Eliminar.Click
        If lstView.SelectedItems.Count > 0 Then
            Dim selectedClientId As String = lstView.SelectedItems(0).Text
            Console.WriteLine("Id_Clientes a eliminar: " & selectedClientId)
            Try
                Using connDB As New MySqlConnection($"Server=localhost;Database={database};Uid={user};Pwd={pass};")
                    connDB.Open()
    ...
    '        If txtBuscaCli.Text <> "" Then
    '            txtBuscaCli.Text = lstView.FocusedItem.Text
    '            exists = System.IO.File.Exists(My.Application.Info.DirectoryPath & "\base.wel")
    '            If exists = False Then
    '                frmBuscarBase.Show()
    '            Else
    '                fileReader = _
    '                My.Computer.FileSystem.OpenTextFileReader(My.Application.Info.DirectoryPath & "\base.wel")
    ...
    '                    comd.CommandText = "UPDATE Clientes SET Estado_Cli='Activo' WHERE Id_Clientes= " & txtBuscaCli.Text
    ...
    '                    comd = New OleDb.OleDbCommand
    '                    comd.Connection = connDB
    '                    comd.CommandText = "UPDATE Clientes SET Estado_Cli='Inactivo' WHERE Id_Clientes= " & txtBuscaCli.Text
    '                    comd.ExecuteNonQuery()
    '                    connDB.Close()
